...
try {
    ...
    # Create the custom ISO using oscdimg
    $oscdimgArgs = @(
        "-n"                    # Allow long file names
        "-m"                    # Ignore maximum image size limit
        "-b$extractDir\isolinux\isolinux.bin"  # El Torito boot sector
        "-c$extractDir\isolinux\boot.cat"      # Boot catalog
        $extractDir             # Source directory
        $customISO              # Output ISO
    )
